...
namespace Kreatys\CmsBundle\Menu;
    ...
    private function addItem($item, $menu, $isChild = false) {
        ...
        if (count($item->getChildren()) > 0) {
            if (!empty($item->getPage())) {
                $menu->addChild($slugLabel, array('route' => $item->getPage()->getRouteName(), 'label' => $label))
                        ->setAttribute('dropdown', true);
            } elseif (!empty($item->getRouteName())) {
                $menu->addChild($slugLabel, array('route' => $item->getRouteName(), 'label' => $label))
                        ->setAttribute('dropdown', true);
            } elseif (!empty($item->getAncre())) {
                $menu->addChild($slugLabel, array('uri' => $this->router->generate($item->getAncre()->getBlock()->getPage()->getRouteName()).'#'.$item->getAncre(), 'label' => $label))
                        ->setAttribute('dropdown', true);
            } else {
                $menu->addChild($slugLabel, array('uri' => $item->getUrl(), 'label' => $label))
                        ->setAttribute('dropdown', true);
            }
            ...
            if (!empty($item->getPage())) {
                ...
                $menu->addChild($slugLabel, array('uri' => $this->router->generate($item->getAncre()->getBlock()->getPage()->getRouteName()).'#'.$item->getAncre()->getNom(), 'label' => $label));
            } else {
                $menu->addChild($slugLabel, array('uri' => $item->getUrl(), 'label' => $label));
            }
        }
